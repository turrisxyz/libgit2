...
void test_repo_open__validates_dir_ownership(void)
{
	...
	git_fs_path__set_owner(GIT_FS_PATH_OWNER_IS_CURRENT_USER);
	cl_git_pass(git_repository_open(&repo, "empty_standard_repo"));
	git_repository_free(repo);

	/* When the system user owns the repo config, fail */
	git_fs_path__set_owner(GIT_FS_PATH_OWNER_IS_ADMINISTRATOR);
	cl_git_fail(git_repository_open(&repo, "empty_standard_repo"));

#ifdef GIT_WIN32
	git_fs_path__set_owner(GIT_FS_PATH_OWNER_AND_USER_ARE_ADMINISTRATOR);
	cl_git_pass(git_repository_open(&repo, "empty_standard_repo"));
	git_repository_free(repo);
#endif

	/* When an unknown user owns the repo config, fail */
	git_fs_path__set_owner(GIT_FS_PATH_OWNER_IS_OTHER);
	cl_git_fail(git_repository_open(&repo, "empty_standard_repo"));
}
...
void test_repo_open__validates_bare_repo_ownership(void)
{
	...
	/*
	 * When the system owns the config and the user is an
	 * administrator, that's also acceptable.
	 */
#ifdef GIT_WIN32
	git_fs_path__set_owner(GIT_FS_PATH_OWNER_AND_USER_ARE_ADMINISTRATOR);
	cl_git_pass(git_repository_open(&repo, "testrepo.git"));
