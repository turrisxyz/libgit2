...
typedef enum {
	...
	GIT_FS_PATH_OWNER_IS_CURRENT_USER = (1 << 0),

	/** The file must be owned by the system account. */
	GIT_FS_PATH_OWNER_IS_ADMINISTRATOR = (1 << 1),
	 ...
	 */
	GIT_FS_PATH_OWNER_AND_USER_ARE_ADMINISTRATOR = (1 << 2),

	/** The file may be owned by another user. */
	GIT_FS_PATH_OWNER_IS_OTHER = (1 << 4)
} git_fs_path_owner_t;
...
/** Verify that the file in question is owned by the specified owner. */
int git_fs_path_owner_is(
	bool *out,
	const char *path,
	git_fs_path_owner_t owner);
